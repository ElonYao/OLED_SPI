...
extern xSemaphoreHandle g_xSPIMutex;
...
inline static void setBuffByte(byte* buff, byte x, byte y, byte val)
{
...
inline static byte readPixels(const byte* loc, bool invert)
{
	...
	byte pixels = pgm_read_byte(loc);  //dè¯»å�–flashé‡Œé�¢çš„æ•°æ�®åˆ°ram
	if(invert)
		pixels = ~pixels;
	return pixels;
}

/* æŠŠnewvalä¸­ä»Žbit0å¼€å§‹çš„num_bitä¸ªæ•°æ�®ä½�,
 * è®¾ç½®è¿›oldvalé‡Œ(ä»Žstart_bitå¼€å§‹)
 * æ¯”å¦‚:
 * oldvalçš„bit[start_bit]è®¾ç½®ä¸ºnewvalçš„bit[0]
 * oldvalçš„bit[start_bit+1]è®¾ç½®ä¸ºnewvalçš„bit[1]
 * ä¿�ç•™oldvalä¸­å…¶ä»–ä½�ä¸�å�˜
 */
...
void draw_bitmap(byte x, byte yy, const byte* bitmap, byte w, byte h, bool invert, byte offsetY)
{
	...
	for (int page = 0 ; page < pages; page++)
	{
		...
		for (int col = 0; col < w; col++)
		{
			...
			/* æŠŠpixelsä¸­çš„bitsä½�è®¾ç½®è¿›oledBuffer */
...
void draw_end()
{
	OLED_Flush(); //åˆ·æ–°å±�å¹•çš„æ„�æ€�, OLEDçš„åˆ·æ–°éœ€è¦�100ms, å¤ªæ…¢äº†
